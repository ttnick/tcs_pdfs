...
\begin{example}
	...
	\begin{itemize}
		\item $L(\bm{((a+b)(a+b))} \overset{\bm{\ast}}{}) = \{ w \in \Sigma^\ast \mid |w| \text{ gerade} \}$.
		\item $L(\bm{(a+b)(a+b)(a+b)} \overset{\bm{\ast}}{}) = \{ w \in \Sigma^\ast \mid |w| \geq 2 \}$.
		\item $L(\bm{a} \overset{\bm{\ast}}{} \bm{+ b} \overset{\bm{\ast}}{}) = \{ w \in \Sigma^\ast \mid |w|_a = 0 \text{ oder } |w|_b = 0 \}$.
	\end{itemize}
\end{example}
...
\end{lemma}
\begin{example}
	Wir betrachtenn den Automaten $\mathcal{A}$ in Abbildung~\ref{fig:state_equivalence}. Es gibt 6 Zustände in $\mathcal{A}$, aber nur 3 $\sim_\mathcal{A}$-Äquivalenzklassen: $\{q_0, q_1, q_5\}, \{q_2, q_3\}, \{q_4\}$.
\end{example}
\begin{figure}
	\centering
	\input{figs/state_equivalence}
	\caption{Ein DFA $\mathcal{A}$ mit 6 Zuständen, aber nur 3 $\sim_\mathcal{A}$-Äquivalenzklassen.}
