...
\begin{definition}
	...
	\]
	\( M \) heißt der \textit{Definitionsbereich} (auch Domäne) von \( f \), \( N \) heißt der \textit{Wertebereich} von \( f \). \( f(x) \) ist das \textit{Bild} von \( x \) unter \( f \) und \( x \) ist ein \textit{Urbild} von \( f(x) \) unter \( f \). Die Menge aller Abbildungen von \( M \) nach \( N \) wird mit \( N^M \) bezeichnet. Falls \( M = A^n, N = A \) für ein nicht-leeres \( A \) ist, sagen wir auch, dass \( f \) eine \textit{\( n \)-stellige} Funktion über \( A \) ist. Desweitere nennen wir eine \( 0 \)-stellige Funktion auch \textit{Konstante}.
\end{definition}
\begin{definition}
...
\end{example*}
Für Abbildungen \( f\colon [k] \to M \) für beliebige \( k \in \mathbb{N} \) in beliebige \( M \) können wir auch abkürzend die Tupelschreibweise \( (y_0, \ldots, y_{k-1}) \) verwenden. Dann ist \( y_i = f(i) \) für alle \( i \in [k] \).\par
Falls wir den Definitionsbereich einschränken wollen schreiben wir benutzen wir folgende Definition.
\begin{definition}
	Sei \( f\colon M \to N \) eine Abbildung und \( A \subseteq M \). Dann ist \( f\vert_A\colon A \to N \) die \textit{Einschränkung von \( f \) auf \( A \)} mit \( f\vert_A(x) = f(x) \) für alle \( x \in A \).
\end{definition}
%TODO id, \equiv, \circ


\subsection{Strukturen}\label{sec:pre_structures}
Wir haben Mengen, Relationen und Abbildungen jeweils eigenständig kennengelernt. Tatsächlich benutzen wir sie allerdings nur zusammen. Beispielsweise sind die natürlichen Zahlen für sich allein nicht sehr interessant, wenn man auf ihnen nicht addieren könnte. Umgekehrt sind auch Funktionsvorschriften wertlos, wenn sie nicht auf Elemente einer Menge angewndet werden kann. Wir fassen alles nun unter dem Begriff einer \textit{Struktur} zusammen.
\begin{definition}
