...
\begin{remark*}
	...
	$$
	Dann ist $\{\Sigma^\ast_q : q \in Q\}$ eine Partition von $\Sigma^\ast$ und die dadurch induzierte Äquivalenzrelation 
	$$
		\sim_\mathcal{A} \,= \{(u, v) \in \Sigma^\ast \times \Sigma^\ast \mid u, v \in \Sigma^\ast_q \text{ für ein } q \in Q\}
	$$
	ist eine Verfeinerung von $\sim_{L(\mathcal{A})}$. Intuitiv: Wörter die im selben Zustand landen sind äquivalent ($\sim_\mathcal{A}$).
\end{remark*}
