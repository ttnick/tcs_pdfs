...
\begin{definition}
	...
	und
	\[
		\lambda: E \to 2^\Sigma, \quad (p, q) \mapsto \{ a \mid \delta(p, a) = q \}.
	\]
\end{definition}
...
\begin{lemma}\label{lem:nfa2dfa}
	...
	\begin{proof}
		...
		Wir zeigen, dass beide Automaten äquivalent sind. Sei \( w \in L(\mathcal{A}) \). Betrachte nun die Erreichbarkeitsmenge von \( \mathcal{A} \) auf allen Präfixen von \( w \).\\
		TODO
	\end{proof}
\end{lemma}
\begin{example}
