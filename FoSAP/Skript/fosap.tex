...
\begin{algorithm}
	...
	\While{$P \neq \emptyset$}{
		$S := P$.dequeue()\\
		\For{$a \in \Sigma$}{
			$R := \{\bigcup_{p \in S} \{ r \mid (p, a, r) \in \Delta \}$\\
			$P$.enqueue($R$)\\
			$Q^\prime := Q^\prime \cup R$\\
			$\delta(S, a) = R$
		}
	}
	\For{$P \in Q^\prime$}{
		\uIf{$P \cap F \neq \emptyset$}{
			$F^\prime := F^\prime \cup \{ P \}$
		}
	}
...
\begin{definition}\label{def:regexsyntax}
	...
	\end{itemize}
	Die Menge aller regulären Ausdrücke über \( \Sigma \) bezeichnen mit \( \mathsf{RE}_\Sigma \).
\end{definition}
...
\begin{definition}
	...
	\[
		\llbracket \cdot \rrbracket: \mathsf{RE}_\Sigma \to 2^{\Sigma^\ast}
	\]
	mit
	\begin{itemize}
